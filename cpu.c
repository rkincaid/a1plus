//////////////////////////////////////////////////////////////////
// 6/2021 R.Kincaid based on the arduino_6502 
// project from miker00lz (Mike Chambers)
// http://forum.arduino.cc/index.php?topic=193216.0
// changed to handle multiple ROM images and
...
//////////////////////////////////////////////////////////////////
// NOTE: Define the available memory for emulated RAM
// if you use a different board be sure to set this as appropriate.
//////////////////////////////////////////////////////////////////
...
uint8_t read6502(uint16_t address) {
  ...
  if (BASICID==2 && (address >= 0xD200) && (address <= 0xFFFF)) {
    return(pgm_read_byte_near(EHBASIC + address - 0xD200));
  }
  
  if ((address >= 0xE000) && (address <= 0xFFFF)) {
    return(pgm_read_byte_near((BASICID?ASOFT:BASIC)+(address-0xE000)));   
  }

  if (address == 0xD010) { //a1 simulated PIA key input
    if(!isakey()) return(0);
    clearkey();
    return(BASICID!=2?getkey()|0x80 : getkey());
  }
...
void write6502(uint16_t address, uint8_t value) {
  ...
  else if (address == 0xD012) { //a1 simulated PIA output
    serout(value & 0x7F);
  }
  else if (address == 0xD020) { //set which basic is active
    BASICID = value;
    reset6502();
    if (BASICID<2) pc = 0xE000;
    if (BASICID==3) {
      BASICID=0;
      pc = 0xFF00;
    }
